    ...
    >
      <Column justifyContent={'space-between'} gap={2} height={'100%'} sx={{ userSelect: 'none' }}>
        <Box borderBottom={`1px solid ${theme.palette.common.black_40}`}>
          <Typography variant='h2' fontWeight={700} paddingBottom={2} marginBottom={2} borderBottom={`1px solid ${theme.palette.common.black_40}`}>
            付款完成
          </Typography>
          {paymentLogs &&
            paymentLogs.map((paymentLog: PaymentLogsResponse) => (
              <Column key={paymentLog.paymentNo} border={'1px solid #d1d1d1'} borderRadius={1.5} marginBottom={2} p={2} gap={1}>
                ...
                <Divider />
                {paymentLog.order?.orderMeals?.map((orderMeal) => (
