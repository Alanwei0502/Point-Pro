...
import { Order, GatherOrder } from "~/features/orders/type";
...
import { DialogType, OrderStatus, SocketTopic } from "~/types";
...
export const getOrders = createAppAsyncThunk(
  `${name}/getOrders`,
  async (payload: { status?: IOrderSliceState["status"] }, { rejectWithValue }) => {
    try {
      const { status = "" } = payload;
      const orderRes = await OrderApi.getOrders({ status });
